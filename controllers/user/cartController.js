    ...
    if (existing) {
      ...
      if (newQty > stockAvailable) {
        return res.status(400).json({
          success: false,
          message: `You already have ${existing.quantity} in your cart. Only ${
            stockAvailable - existing.quantity
          } more can be added for size ${normalizedSize.toUpperCase()}.`,
        });
      }
...
const updateCartItem = async (req, res) => {
  try {
    ...
    if (product.shoeSizes && product.shoeSizes.has(sizeKey)) {
      ...
      product.variant.size[sizeKey]
    ) {
      stockAvailable = product.variant.size[sizeKey];
    } else if (product.sizes) {
      const sizeObj = product.sizes.find(
        (s) => s.size.toLowerCase() === sizeKey
      );

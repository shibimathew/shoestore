...
const editProfileInfo = async (req, res) => {
  try {
    ...
    if (!fullName || !email || !gender) {
      return res
        .status(400)
        .json({ message: "Full name, email, and gender are required" });
    }
    ...
    if (req.file) {
      ...
      if (!validMimes.includes(file.mimetype)) {
        return res
          .status(400)
          .json({ message: "Only JPG, JPEG,PNG and WEBP images are allowed" });
      }
...
const renderSecurityPage = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .render("error", { message: "Server error occurred: " + error.message });
  }
};

...
const getMyAddresses = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .render("user/login", { message: "Please login to view addresses" });
    }
    ...
    res
      .status(500)
      .render("error", { message: "Server error while fetching addresses" });
  }
};
...
const getAddMyAddressesPage = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .render("user/login", { message: "Please login to add addresses" });
    }
...
const addMyAddresses = async (req, res) => {
  try {
    ...
    if (
      altPhone &&
      altPhone.toString().trim() &&
      !/^[6789]\d{9}$/.test(altPhone.toString().trim())
    ) {
      return res.status(400).json({
        success: false,
